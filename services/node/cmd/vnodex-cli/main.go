...
func usage() {
	fmt.Println("vnodex-cli commands: status | events [limit] | state <entity> | snapshot [scope] | submit | keygen")
}
...
func sampleEvent(args []string) model.Event {
	...
	core := model.EventCore{
		...
		SignerID:               entity,
		SignerPublicKey:        "",
		ValidationProof:        "dev-local",
	}
	hash, _, _ := model.CanonicalEventHash(core)
	return model.Event{EventCore: core, EventHash: hash, Signature: "", Certified: true, Accepted: true}
}
